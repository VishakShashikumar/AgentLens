...
**AgentLens profiles, evaluates, and optimizes other LLM agents.** Point it at
an agent, give it a golden-task benchmark, and it traces every tool call and
token, diagnoses *why* the agent is inefficient using a named taxonomy of
...
This is not another agent that books flights or answers support tickets.
Most agent demos are, and they're getting commoditized fast. AgentLens is
the tool that watches those agents run and tells you what's actually wrong
with them — the skill set behind it (evaluation design, tracing/observability,
tool orchestration, cost optimization) is what 2026 AI-engineering hiring
guides call the single strongest signal that separates someone who's actually
built with LLMs from someone who's watched a tutorial.

## A field guide to agent failure patterns

This is the core idea, and the part worth leading with in an interview. Four
named, detectable patterns, each with a concrete fix:
...
pytest                                # 10 tests: tracer, checkers, diagnoser,
...
Lead with the diagnosis, not the demo. Walk through one specific pattern you
found — say, Retry Storm on the coding agent — and the exact fix AgentLens
applied, with the real before/after numbers from the table above. Be ready
to explain the mock-provider design decision above unprompted: it shows you
thought about testability and cost, not just about getting a demo to run
once. Have the golden-task design and the judge's scoring logic ready to
defend in detail — that's the part that separates this from a weekend
tutorial project.
